...
impl QuantizedFlatTree {
    fn quantize_hyperplanes(tree: &FlatTree) -> (Vec<i8>, Vec<f32>) {
        ...
        for hyperplane in tree.hyperplanes.chunks_exact(tree.dim) {
            let max_abs = hyperplane.iter().fold(0.0f32, |value, &x| value.max(x.abs()));
            let scale = if max_abs > 0.0 { max_abs / 127.0 } else { 0.0 };
            scales.push(scale);
            hyperplanes.extend(hyperplane.iter().map(|&x| {
                if scale > 0.0 { (x / scale).round().clamp(-127.0, 127.0) as i8 } else { 0 }
            }));
        }
        (hyperplanes, scales)
    }
    ...
    pub fn search_leaves(
        ...
        while leaves.len() < leaf_budget && !frontier.is_empty() {
            let best = frontier.iter().enumerate()
            ...
            frontier.push((priority.max(margin.abs()), self.children[node][1 - side] as usize));
        }
        evaluations
    }

    pub fn code_bytes(&self) -> usize { self.hyperplanes.len() }
    pub fn scale_bytes(&self) -> usize { self.scales.len() * std::mem::size_of::<f32>() }
    pub fn offset_bytes(&self) -> usize { self.offsets.len() * std::mem::size_of::<f32>() }
}
...
mod tests {
    ...
    fn test_spill_search_leaf_budget() {
        ...
        let evaluations = tree.search_leaves(
            &[1.0, 0.0],
            &mut rng,
            2,
            &mut frontier,
    ...
    fn test_quantized_margin_reconstruction_sign_and_odd_dimension() {
        ...
        let expected = quantized_i8_dot(&quantized.hyperplanes, &query) as f32
            * quantized.scales[0] * query_scale + 0.2;
    ...
    fn test_generated_angular_tree_quantized_routing_fidelity() {
        ...
        let mut data: Vec<f32> = (0..n * dim)
            .map(|i| ((i * 37 + 11) as f32 * 0.017).sin()
                + 0.31 * ((i * 13 + 5) as f32 * 0.029).cos())
            .collect();
        for row in data.chunks_exact_mut(dim) {
            let norm = row.iter().map(|x| x * x).sum::<f32>().sqrt();
            for value in row { *value /= norm; }
        }
        ...
        for query_id in 0..128 {
            ...
            if fp_leaves == q_leaves { identical += 1; }
            let common = fp_leaves.iter().filter(|leaf| q_leaves.contains(leaf)).count();
