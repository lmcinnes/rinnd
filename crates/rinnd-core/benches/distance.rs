...
use rinnd_core::distance::quantized::{quantized_i8_alternative_dot, quantized_i8_dot};
use rinnd_core::distance::{
    AlternativeDot, Cosine, DirectNormalizedCosine, Distance, Euclidean, InnerProduct,
    SquaredEuclidean,
};
...
fn bench_euclidean(c: &mut Criterion) {
    ...
    for dim in [32, 64, 128, 256, 512, 768, 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", dim), dim, |bench, _| {
            bench.iter(|| black_box(Euclidean.distance(&a, &b)))
        });
    }
...
fn bench_cosine(c: &mut Criterion) {
    ...
    for dim in [32, 64, 128, 256, 512, 768, 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", dim), dim, |bench, _| {
            bench.iter(|| black_box(Cosine.distance(&a, &b)))
        });
    }
...
fn bench_inner_product(c: &mut Criterion) {
    ...
    for dim in [32, 64, 128, 256, 512, 768, 1024].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", dim), dim, |bench, _| {
            bench.iter(|| black_box(InnerProduct.distance(&a, &b)))
        });
    }
...
fn bench_quantized_i8_dot(c: &mut Criterion) {
    ...
    for dim in [100, 128, 784] {
        ...
        group.bench_with_input(
            BenchmarkId::new("alternative_dot", dim),
            &dim,
            |bench, _| {
                bench.iter(|| {
                    black_box(quantized_i8_alternative_dot(
                        black_box(&a),
                        black_box(&b),
                        inv_norm_a,
                        inv_norm_b,
...
fn bench_random_candidate_angular(c: &mut Criterion) {
    ...
    let query_inv_norm = (quantized_i8_dot(quantized_query, quantized_query) as f32)
        .sqrt()
        .recip();
