...
pub use inner_product::{Dot, InnerProduct};
pub use traits::{Distance, HasSquaredForm};

// Minkowski family
pub use minkowski::{BrayCurtis, Canberra, Chebyshev, Manhattan, Minkowski};

// Binary/set distances
pub use binary::{
    correct_alternative_jaccard, AlternativeJaccard, BitHamming, BitJaccard, Dice, Hamming,
    Jaccard, Kulsinski, Matching, RogersTanimoto, RussellRao, SokalMichener, SokalSneath, Yule,
};

// Alternative/proxy distances
pub use alternatives::{
    correct_alternative_cosine, correct_alternative_inner_product, true_angular_from_alt_cosine,
    AlternativeCosine, AlternativeDot, AlternativeInnerProduct, DirectNormalizedCosine,
    ProxyInnerProduct,
};

// Hellinger and distribution distances
pub use hellinger::{
    correct_alternative_hellinger, AlternativeHellinger, Hellinger, JensenShannon,
    ProxyCircularKantorovich, ProxyJensenShannon, ProxyKantorovich, ProxySymmetricKL,
    ProxyWasserstein1D, SymmetricKL,
};
...
mod tests {
    ...
    fn test_metric_from_str() {
        ...
        assert_eq!(
            Metric::from_str("jensen-shannon"),
            Some(Metric::JensenShannon)
        );
